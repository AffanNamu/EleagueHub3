...
/**
 * The actual profile page content, shared by both routes that can land a
 * visitor on someone's profile:
 *   - /profile/[id]  (routeUid = the uid straight from the URL segment)
 *   - /u/[username]  (routeUid = resolved from the username first, by the
 *     caller, then handed down here — see app/u/[username]/page.tsx)
 *
 * Extracted out of app/(dashboard)/profile/[id]/page.tsx so the pretty
 * username route doesn't have to duplicate ~350 lines of profile UI, and
 * — critically — doesn't have to redirect to /profile/{uid} to reuse it,
 * which would defeat the entire point of a pretty link. Mirrors mobile's
 * UsernameProfileGateScreen, which deliberately renders
 * PublicTeamProfileScreen directly under the /u/:username URL rather than
 * navigating away from it.
 */
export function ProfileDetailView({ routeUid }: { routeUid: string }) {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      setAuthUser(user);
      setTargetUid(routeUid || user?.uid || '');
                ...
                <>
                  ...
                  {showMoreMenu && (
                    <div className="absolute top-14 right-0 w-48 bg-[#0F172A] border border-white/10 rounded-2xl shadow-2xl py-2 z-20">
                      <button onClick={() => { const link = usernameLower ? `${window.location.origin}/u/${usernameLower}` : window.location.href; navigator.clipboard.writeText(link); alert('Link Copied'); setShowMoreMenu(false); }} className="w-full px-4 py-3 text-left text-sm font-bold text-gray-300 hover:bg-white/5 flex items-center gap-2"><LinkIcon className="w-4 h-4"/> Copy Profile Link</button>
