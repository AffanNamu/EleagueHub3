...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const item = await updateMarketplaceProduct(params.id, parseInput(body), {
      uid: identity!.uid,
    ...
    console.error('[update marketplace product]', err);
...
export async function DELETE(_request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    await deleteMarketplaceProduct(params.id, { uid: identity!.uid, email: identity!.email });
    return NextResponse.json({ ok: true });
  } catch (err) {
    if (err instanceof MarketplaceProductError) {
      return NextResponse.json({ error: err.message }, { status: 400 });
    }
    console.error('[delete marketplace product]', err);
    return NextResponse.json({ error: 'Something went wrong. Please try again.' }, { status: 500 });
  }
}
