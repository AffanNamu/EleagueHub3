...
// LiveKit client uses ws(s) for signaling; server APIs require http(s).



// Accept either and normalize for REST/Twirp calls.
...
authorization: Bearer ${await makeAdminJwt(env, body.room)},
...
// protobuf JSON enums are commonly strings ("AUDIO") but handle numbers defensively.



const t = trackInfo?.type;
...
if (t === 0) return true; // TrackType.AUDIO is 0 in LiveKit protos
...
Mute/unmute ALL published audio tracks for an identity (no trackSid needed from clients).
...
const participants = Array.isArray(lp?.participants) ? lp.participants : [];
...
.filter((t) => isAudioTrackType(t) && typeof t?.sid === "string" && t.sid.length > 0)
...
if (url.pathname === "/" && request.method === "POST") {
  ...
  // accept either isHost or role, but prefer isHost
  ...
  // Critical: everyone must be allowed to publish from the start so clients can

  // publish a single mic track once (muted), and later only unmute it to speak.
