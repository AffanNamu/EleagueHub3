...
class WebDesktopSessionStore {
  ...
  static Future<Map<String, String>?> load() async {
    final sessionId =
        html.window.localStorage[_kSessionId]?.trim() ?? '';
    final sessionSecret =
        html.window.localStorage[_kSessionSecret]?.trim() ?? '';
    final pairedUserUid =
        html.window.localStorage[_kPairedUserUid]?.trim() ?? '';

    // Need at minimum a paired user uid to restore session
    ...
    return <String, String>{
      ...
      'pairedUserName':
          html.window.localStorage[_kPairedUserName] ?? '',
      'pairedUserEmail':
          html.window.localStorage[_kPairedUserEmail] ?? '',
    };
  }
