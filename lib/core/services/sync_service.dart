...
class SyncService {
  ...
  int _desiredInitialCouponCount({
    required Map<String, dynamic> leaguePayload,
  }) {
    final couponCount = _intFromAny(leaguePayload['couponCount'], fallback: 0);
    if (couponCount > 0) {
      // Created via payment screen (explicit purchase quantity).
      return couponCount.clamp(1, 500);
    }

    final maxTeams = _intFromAny(leaguePayload['maxTeams'], fallback: 0);
    return (maxTeams > 0 ? maxTeams : 20).clamp(1, 200);
  }

  Future<int> _countExistingCoupons(DocumentReference<Map<String, dynamic>> leagueRef) async {
    try {
      final agg = await leagueRef.collection('coupons').count().get();
      return agg.count ?? 0;
    } catch (_) {
      // Fallback (older SDKs or unexpected errors): do a small probe.
      final existing = await leagueRef.collection('coupons').limit(1).get();
      return existing.docs.isEmpty ? 0 : 1;
    }
  }
  ...
  Future<void> _maybeAutoGenerateInitialCoupons({
    ...
    final desiredCount = _desiredInitialCouponCount(leaguePayload: leaguePayload);

    // Idempotent and self-healing:
    // - If coupons already exist but are fewer than desired (partial generation),
    //   generate only the missing remainder.
    final existingCount = await _countExistingCoupons(leagueRef);
    if (existingCount >= desiredCount) return;

    final remaining = desiredCount - existingCount;

    final now = DateTime.now().millisecondsSinceEpoch;

    // Update league trace fields (best-effort).
    await leagueRef.set(
      <String, dynamic>{
        'couponsEnabled': true,
        'couponDiscountPercent': discountPercent,
        'couponCount': _intFromAny(leaguePayload['couponCount'], fallback: 0),
        'couponsAutoGeneratedAtMs': now,
        'couponsAutoGeneratedTargetCount': desiredCount,
        'updatedAtMs': now,
      },
    ...
    while (generated < remaining) {
      final toGenerate = (remaining - generated).clamp(1, batchSize);
      final batch = _firestore.batch();

      for (int i = 0; i < toGenerate; i++) {
        // Guard against extremely unlikely repeated collisions.
        if (guard++ > remaining + 50) {
          break;
        }
    ...
    // Store how many we tried to generate in this sync run.
    await leagueRef.set(
      <String, dynamic>{
        'couponsAutoGeneratedCount': (existingCount + generated),
        'updatedAtMs': now,
      },
      SetOptions(merge: true),
    );
