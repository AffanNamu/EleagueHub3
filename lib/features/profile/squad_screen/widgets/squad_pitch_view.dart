...
/// according to the squad's formation. Players that have been explicitly
/// positioned (via drag) render at their own stored x/y; unpositioned
/// players fall back to the formation template's slot position.
class _Pitch extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final maxCount = starting.length > slots.length ? starting.length : slots.length;
            ...
            builder: (context, constraints) {
              ...
              return Stack(
                children: List.generate(maxCount, (i) {
                  ...
                  final nx = hasOwnPosition ? player!.x : (slot?.x ?? 0.5);
                  final ny = hasOwnPosition ? player!.y : (slot?.y ?? 0.5);
                  ...
                  final markerWidth = dense ? markerSize + 30 : markerSize + 24;

                  final marker = PitchPlayerMarker(
                    slotLabel: slot?.label ?? player?.position ?? '',
                    player: player,
                    isCaptain: isCaptain,
