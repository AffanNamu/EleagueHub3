...
import '../../profile/data/match_stats_service.dart';
...
class LocalLeaguesRepository {
  ...
  Future<void> _joinLeagueCore({
    ...
    // ── After the memberIds arrayUnion write in _joinLeagueCore succeeds:
    unawaited(MatchStatsService().incrementCompetitionsJoined(authUid));
  ...
  Future<void> saveLeague(League league) async {
    try {
      ...
      // ── After `await batch.commit()...` in saveLeague / createLeagueLocally:
      unawaited(MatchStatsService().incrementCompetitionsJoined(authUid));
    } catch (e) {
      _rethrowFriendly(
        e is Object ? e : Exception('unknown'),
  ...
  Future<void> updateMatchScoreAndUpdateTeamAggregates({
    ...
    try {
      ...
      // Captured inside the transaction so the post-hoc stats write
      // below has the team names/owners without a second round-trip.
      String homeId = '';
      String awayId = '';
      String homeName = '';
      String awayName = '';
      ...
      await _firestore.runTransaction((txn) async {
        ...
        homeId = (matchData['homeTeamId'] as String? ?? '').trim();
        awayId = (matchData['awayTeamId'] as String? ?? '').trim();
        ...
        homeName = (homeData['name'] as String? ?? '').trim();
        awayName = (awayData['name'] as String? ?? '').trim();
        homeOwnerId = (homeData['ownerId'] as String? ?? homeId).trim();
        awayOwnerId = (awayData['ownerId'] as String? ?? awayId).trim();
      ...
      // Best-effort: fetch league name once for the cache entry, then
      // write both users' recent_matches + stats. Never blocks or
      // throws back to the caller — this mirrors saveTeams' non-fatal
      // post-hoc membership writes above.
      try {
        ...
        leagueName = (leagueSnap.data()?['name'] as String? ?? '').trim();
      } catch (_) {}

      await MatchStatsService().recordMatchResult(
        leagueId: leagueId,
        leagueName: leagueName.isEmpty ? 'League' : leagueName,
        matchId: matchId.trim(),
        homeTeamId: homeOwnerId,
        homeTeamName: homeName.isEmpty ? 'Home' : homeName,
        awayTeamId: awayOwnerId,
        awayTeamName: awayName.isEmpty ? 'Away' : awayName,
        homeScore: hsNew,
        awayScore: asNew,
        playedAtMs: nowMs,
      );
